...
### Phase 8: validated AI evidence interpretation

`src/evidence/llmEvidence.ts` isolates optional direct LLM calls behind an `EvidenceInterpreter` interface. The model receives only explicitly linked request message/image evidence and extracts structured claims such as amounts, dates, statuses, currencies, references, confidence, and explanations. It never makes affordability decisions, changes balances or preferences, creates payment plans, or overrides deterministic challenge rules.

Responses are validated with a strict Zod schema and then reconciled deterministically using explicit cancellation/settlement/amendment precedence, source recency, confidence, and stable identifiers. Invalid references, dates, amounts, malformed JSON, API failures, missing keys, and ambiguous output fall back to no usable AI evidence. Source content is explicitly marked untrusted in the prompt, so embedded instructions remain data. The current direct transport sends image metadata only; image understanding requires a provider implementation that supports image input and must remain behind the same interface.
