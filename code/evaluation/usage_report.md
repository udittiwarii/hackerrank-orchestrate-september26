# Final Run Token Usage Report

Complete this template from the final full-dataset run that produces `output.csv`. Do not record credentials or invent values.

## Model provider(s)

TODO

## Model name(s)

TODO

## Number of model calls

TODO

## Input tokens

TODO

## Output tokens

TODO

## Total tokens

TODO

## Average tokens per request

TODO

## Estimated total cost

TODO

## Estimated cost per request

TODO

## Notes

TODO: Record the run command, dataset scope, pricing assumptions, and any per-model breakdown.

No external AI/LLM calls were made during the data-layer implementation phase.
